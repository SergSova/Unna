$(window).on('load', function () {
  ...
  if (path == '///') {
    //action
    wrap.find('.action').offset({
      top: wrap.find('.panel-3').offset().top
    });
    //more-btn
    wrap.find('.btn-more').offset({
      top: wrap.find('.btn-more-wrap').offset().top - (wrap.find('.btn-more-wrap').height() - wrap.find('.btn-more').height()) / 2,
      left: wrap.find('.btn-more-wrap').offset().left - wrap.find('.btn-more').width() - 100
    })
      .css('bottom', 'auto');
  }

  if (path == '/kontaktyi.html') {
  ...
  if (wrap.find('.content-text').length > 0) {
    wrap.find('p').html();
  }
...
function menuApartItemsAnim() {
  ...
  circleGo.prototype.move = function () {
    ...
    setTimeout(function () {
      this.requestAnim = window.requestAnimationFrame(function () {
        that.move();
      });
    }.bind(this), 50);
  }
